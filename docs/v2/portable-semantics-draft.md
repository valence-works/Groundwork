...
The standing test uses 40 accepted edge rows, four rejected inputs, and exactly 300 unique query
...
| Decimal values | Normalize | Decimal predicates use a declared `decimal(18,4)` domain and compare numerically. Excess scale and values up to `decimal.MaxValue` that exceed the declared precision fail closed; values are never rounded implicitly. | Precision and scale must be declared and validated before I/O. | SQLite dynamic typing and provider coercion otherwise admit different values or comparisons. |
| Mixed logical/physical types | Refuse | A numeric logical field backed by string or untyped JSON/BSON storage is an explicit rejected candidate before provider I/O. | Callers must declare a typed numeric projection. | MongoDB cross-BSON numeric comparison, SQLite dynamic typing, and relational coercion do not form one contract. The native four-provider probe demonstrates the divergence; the standing gate asserts the refusal. |
...
| Supported negation | Normalize | `NotEqual` is the total complement of equality for text, number, boolean, date/time, and GUID, including null. | Providers must render an explicit null branch rather than expose SQL three-valued inequality. | Equality and inequality must partition the corpus. |
| Unsupported negation | Refuse | `NotContains` is refused for the scale-bearing route above. `NotIn`, `NotStartsWith`, and separate negated-range operators are refused from the v2 AST until new differential evidence supports them; the v1 query enum has no shapes for them. | No provider-specific synthesized rewrite is allowed. | An absent operator cannot become a silent third outcome. |
