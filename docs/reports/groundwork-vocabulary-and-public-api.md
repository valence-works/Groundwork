...
- The route-native `Groundwork.Core.SchemaEvolution` model is the canonical desired-state diff, durable-state, locking, and safe-application contract introduced by #44. The former compatibility materialization package remained a planner/executor surface until providers moved to executable routes in #46–#48. The unused duplicate `Groundwork.Core.Materialization` records and the independent imperative migration pipeline still required convergence.
...
| Current public API | Runtime role verified in source | Assessment |
|---|---|---|
| Former compatibility `MaterializationPlan` | Compatibility provider plan formerly used by provider materializers and document-store factories. Its additive-index rebuild became an explicit canonical-JSON backfill operation. | Removed after #46–#48 moved providers to route-native schema diffs. |
| [`Groundwork.Core.SchemaEvolution`](../../src/Groundwork/Core/SchemaEvolution) | Deterministic additive diff from executable routes to canonical durable applied state, plus lock/executor/application contracts. | Canonical #44 desired-state and safe-application boundary. Providers translate its semantic operations in #46–#48. |
| Former compatibility `MaterializationOperation` | Compatibility operations for storage-unit creation, index creation, optimized projection creation, and schema-history recording; canonical-JSON backfill later used the Core schema-evolution operation directly. | Removed after #46–#48; no second backfill authority remains. |
| Former compatibility `MaterializationPlanner` | Validated a manifest and provider reports, derived create operations, and emitted schema history. | Removed after route-native schema diffs became authoritative. |
| Former compatibility `SchemaHistoryEntry` | Carried manifest identity/version, provider, planning time, and operation targets. | Removed after durable route-native applied state took ownership. |
| `Groundwork.Core.Materialization` plan, operation, enum, and schema-history records | No repository runtime usage outside their own declarations. They duplicated the former compatibility package's names with different shapes. | Removed at the breaking boundary. They contradicted ADR 0001's ownership decision. |
...
5. `ExecutableStorageRouteCompiler` freezes provider physical definitions into immutable executable routes.
6. `PhysicalSchemaDiffPlanner` compares those routes with durable `PhysicalSchemaAppliedState` and emits stable additive creation, projection, index, canonical-JSON-backfill, validation, and recording operations.
7. `PhysicalSchemaApplication` executes through `IPhysicalSchemaExecutor` under one provider/manifest exclusion lease and records state only after exact durable acknowledgements. Provider translation arrives with #46–#48.
...
`Groundwork.Core.SchemaEvolution` owns the route-native desired-state diff and safe-application contracts. The former compatibility provider plan remained only until #46–#48 and scheduled the Core canonical-JSON-backfill operation directly. The subsequent vocabulary cleanup removed the duplicate `Groundwork.Core.Materialization` records and compatibility contracts rather than maintaining parallel authorities indefinitely.
...
| Former compatibility materialization types | Retired after route-native schema evolution became the only preparation/evolution plan. |
