...
- [x] T003 Define provider-owned active/candidate transition state and legal phase transitions — Evidence: `SqliteRelationshipTransitionExecutor` persists Preparing, Validated, Active, and Failed state with candidate-bound expected-active preconditions.
...
- [x] T007 Reject dangling legacy references with candidate-bound `GW-RELATIONSHIP-013` and no raw data — Evidence: focused SQLite test verifies opaque candidate-bound diagnostic, no activation, and no dangling sidecar/fence rows.
- [x] T008 Persist bounded progress and make same-request backfill/revalidation replay-safe — Evidence: cancellation test leaves `processed_source_count` durable and a distinct executor resumes; acknowledgement-loss tests converge.
...
- [x] T011 Recover deterministically across distinct-instance reopen before validation, after validation, and after cutover acknowledgement loss — Evidence: focused suite covers bounded-progress cancellation, validated-state acknowledgement loss, and post-cutover acknowledgement loss with distinct executors.
...
- [x] T013 Add real durable SQLite tests for valid backfill, dangling failure, cutover competition, replay, cancellation, and restart — Evidence: `SqliteRelationshipTransitionTests` has eight file-backed SQLite scenarios, including concurrent competition, exact rotations, and distinct-instance reopening.
...
- [x] T015 Run focused/Core/materialization/provider gates and record exact evidence and nonclaims in the quickstart — Evidence: quickstart records 8 SQLite, 50 Core, and 1 materialization relationship test passing on this branch.
