...
/// two with a category and one with none — and a scale-bearing query bound to that index whose predicate
/// no provider can prove rejects nulls. Row exclusion is a storage choice, so the answer has to be either
/// every row the predicate matches or a refusal that names the index; what it may never be is a short
/// result set, which is what MongoDB returned before the guard was shared.
/// </para>
/// <para>
/// Providers legitimately disagree about which branch they take, because they disagree about whether the
/// index is load-bearing: SQL Server, SQLite, and MongoDB pin it and therefore refuse, while PostgreSQL
...
public static class NullExcludedIndexConformance
{
    ...
    /// <summary>The predicate every provider can prove rejects nulls, and therefore must always serve.</summary>
    public static DocumentQueryComparison Provable => DocumentQueryComparison.Equal(CategoryPath, "alpha");

    /// <summary>
    /// The predicates no provider can prove reject nulls, each with the rows it matches: a null equality
    /// asks for exactly the documents the index omits, and a membership set containing null asks for them
    /// alongside others. <c>NotContains</c> is the third shape and behaves the same way, but MongoDB
    /// refuses to certify it on a scale-bearing route at all — its case-insensitive regular expression
    /// cannot be served by a B-tree index — so it cannot appear in a manifest every provider admits.
    /// </summary>
